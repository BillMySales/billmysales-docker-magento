#!/bin/bash
# Backups of the database, media and app/etc/env.php.
#
#   backup.sh            # loop: back up now, then every BACKUP_INTERVAL_HOURS
#   backup.sh now        # one backup
#   backup.sh list       # list backups
#   backup.sh restore <timestamp>   # restore DB, media and env.php from a backup
...
# (pub/media and app/etc/env.php, which holds the encryption key; the code
...
FILES="app/etc/env.php pub/media"
...
restore() {
    ...
    echo "==> Restoring media and env.php from ${files}"
    find "${APP_DIR}/pub/media" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
    tar -C "${APP_DIR}" -xzpf "${files}"
    echo "    Then run: docker compose run --rm setup (cache flush) and reindex if needed"
    echo "==> Restored ${ts}"
}
